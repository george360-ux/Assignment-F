...
The graphs below show comparisons between conventional and organic strawberry production. Both graphs show production over time in years (x-axis). The first graph shows the number of acres harvested by state. The second graph shows the amount of strawberries produced per one hundred pounds. 

```{r}
#| label: acres harvested graph conventional
#| warning: false
#| echo: false

harvest_data_c <- strawb_survey %>%
  filter(`Data Item` == "ACRES HARVESTED")

p3 <- ggplot(data = harvest_data_c, aes(x = Year, y = Value, color = State)) +
  geom_line(size = 1) +
  labs(
    title = "Conventional",
    x = "Year",
...
```{r}
#| label: acres harvested graph organic
#| warning: false
#| echo: false

harvest_data_o <- strawb_organic %>%
  filter(`Data.Item` == "ACRES HARVESTED")

p4 <- ggplot(data = harvest_data_o, aes(x = Year, y = Value, color = State)) +
  geom_line(size = 1) +
  labs(
    title = "Organic",
    x = "Year",
...
```{r fig.align='center'}
#| label: acres harvested graphs combo
#| warning: false
#| echo: false
...
(p3 | p4) + 
  plot_annotation(title = 'Strawberry Acres Harvested by State',
