...
```{r}
production_data <- strawb_organic %>%
  filter(`Data.Item` == "PRODUCTION, MEASURED IN CWT")

ggplot(data = production_data, aes(x = Year, y = Value, color = State)) +
  geom_line(size = 1) +
  labs(
    title = "Organic Strawberry Production (CWT) by State",
    x = "Year",
    y = "Production (CWT)",
    color = "State"
  ) +
  theme_minimal() 
  # Ensures the X-axis years are treated as discrete steps if appropriate
  #scale_x_continuous(breaks = unique(production_data$Year)) 
